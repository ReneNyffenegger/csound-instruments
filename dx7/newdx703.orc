...
output:
	...
	a5rand   =       a5rand+1
        a5phs   phasor  i5hz
        a5sig   tablei  (a5phs*a5rand)+a6sig,1,1,0,1
        a5sig   =       a5sig*k5gate

        a4rand   randi   .1,.13
	a4rand   =       a4rand+1
        a4phs   phasor  i4hz
        a4sig   tablei  (a4phs*a4rand)+a5sig,1,1,0,1
        a4sig   =       a4sig*k4gate
	...
	a2rand   =       a2rand+1
        a2phs   phasor  i2hz
        a2sig   tablei  (a2phs*a2rand)+a3sig,1,1,0,1
        a2sig   =       a2sig*k2gate

        a1rand   randi   .011,.1
	a1rand   =       a1rand+1
        a1phs   phasor  i1hz
        a1sig   tablei  (a1phs*a1rand)+a2sig,1,1,0,1
        a1sig   =       a1sig*k1gate

        imsa     =       7
	imsb     =       17
        a1delaya oscil 	imsa/2, 1/p3, 99
	a2delaya oscil  imsa/2, 1/p3, 99   
        a1delaya = 	a1delaya + imsa/2  ;make the delay positive
        a2delaya = 	a2delaya + imsb/2  ;make the delay positive
        aout   =  	(a1sig + a4sig) * ipkamp
        a1delayb vdelay  aout, a1delaya, imsa
	a2delayb vdelay  aout, a2delaya, imsb
	out     (.5*a1delayb) + (.5*a2delayb) 
