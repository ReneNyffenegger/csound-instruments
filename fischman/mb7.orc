; MB7.ORC                Filters the output of any soundfile (file code SOUNDIN.NNN)
;                        with time-varying pulsation frequency, 
;                        filter centre frequency and bandwidth.
;                        Crossfades between scaling and balanced output
;                        Process a stereo soundfile
...
; p19:  CROSSFADE FUNCTION, PROPORTIONAL TO BALANCED OUTPUT, INVERSE TO SCALED
;               OUTPUT
;--------------------------------------------; INITIALIZATION BLOCK
  ...
  ipffluc   =  p10-p9                             ; NULSATION FREQUENCY FLUCTUATIOP
...
;--------------------------------------------; PROCESS
  al,ar     soundin   isfcode,isfskip             ; INPUT SOUNDFILE
  afiltl    reson     al,kcf,kbw,iampbal          ; FILTER LEFT CHANNEL
...
;--------------------------------------------; OUTPUT 
  kxf       oscil1    0,inenv,idur,ixfunc         ; CROSSFADE FUNCTION 
  aoutl     =  kxf*aball+(1-kxf)*afiltl*iampf     ; LEFT CHANNEL CROSSFADE
  aoutr     =  kxf*abalr+(1-kxf)*afiltr*iampf     ; RIGHT CHANNEL CROSSFADE
            outs      kenv*aoutl,kenv*aoutr       ; OVERALL ENVELOPE AND OUTPUT      
endin
