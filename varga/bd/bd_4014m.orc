...
/*-------------------------------------------------------------------------- */
  ...
  ibw01     table     5, 64    /* OSC1 BP bandwidth (rel.)                     */
  ihp1      table     6, 64    /* OSC1 HP frequency (rel.)                     */
  iapf1     table     7, 64    /* OSC1 allpass filter start freq. (rel.)       */
  iapf2     table     8, 64    /* OSC1 allpass filter end freq.                */
  iapdx     table     9, 64    /* OSC1 allpass filter envelope speed           */
  ifr3      table     10, 64    /* OSC2 HP1 frequency / base frq.               */
  imx3      table     11, 64    /* OSC2 HP1 mix                                 */
  ifr4      table     12, 64    /* OSC2 output HP frq. / base freq.             */
  imx2      table     13, 64    /* OSC2 output gain                             */
  ihp2      table     14, 64    /* output HP (OSC1+OSC2) frq. (rel. to note f.) */
  ibw02     table     15, 64    /* output HP resonance                          */
  ifr1      table     16, 64    /* output LP start freq. 1 (rel.)               */
  ifdx1     table     17, 64    /* output LP freq. 1 envelope decay speed       */
  ifr2      table     18, 64    /* output LP start freq. 2                      */
  ifdx2     table     19, 64    /* output LP freq. 2 envelope decay speed       */
  ifxBP1    table     20, 64    /* noise BP start frequency (Hz)                */
  ifxBP2    table     21, 64    /* noise BP end frequency                       */
  ibw1      table     22, 64    /* noise BP bandwidth/freq.                     */
  ifxLP1    table     23, 64    /* noise LP start frequency (Hz)                */
  ifxLP2    table     24, 64    /* noise LP end frequency                       */
  ifxd      table     25, 64    /* noise filter envelope speed                  */
  iattn1    table     26, 64    /* noise attack time (sec.)                     */
  idecn1    table     27, 64    /* noise decay speed                            */
  imxn      table     28, 64    /* noise mix                                    */
...
/*-------------------------------------------------------------------------- */
  ...
  i002      =  exp(log(i001)*(irel+16/kr)/irel)
  ivol      =  ivel*ivol
...
start01:
  ...
  kfn       limit     $CPS2MIDI(kfrq),0,127                     /* oscillator */
  kfn       =  int(kfn+256.5)
  a_        phasor    kfrq
  a1        tableikt  a_,kfn,1,0.0,1
  a2        tableikt  a_,kfn,1,0.5,1
  a1        =  (a1-a2)*16384
  ...
  a0y       wrap      a0x / 40000.0, -1.0, 1.0
  a0x       =  a0x - (a0y*a0y*a0y) * 2000.0
